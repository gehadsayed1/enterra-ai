<template>
  <div :class="[
    'h-screen bg-white shadow-md transition-all duration-300 flex flex-col justify-between',
    isOpen ? 'w-60' : 'w-16',
  ]">
    <div class="mt-6 px-2 flex flex-col gap-2">
      <button @click="isOpen = !isOpen" :class="[
        'p-2  cursor-pointer mb-3  flex  transition-all',
        isOpen ? 'justify-between' : 'justify-center ',
      ]">

        <span v-if="isOpen" class="text-primary text-2xl font-bold">
          ENTERRA AI</span>
        <svg width="25" height="25" viewBox="0 0 20 20" fill="currentColor" xmlns="http://www.w3.org/2000/svg"
          data-rtl-flip="" class="max-md:hidden">
          <path
    ...
    <div class="p-3 mt-auto mb-4">
      <div class="relative">
        <div class="flex items-center gap-3 cursor-pointer" @click="toggleMenu">
          <img v-if="isOpen" src="https://i.pravatar.cc/100" class="w-10 h-10 rounded-full object-cover" />

          <img v-else src="https://i.pravatar.cc/100" class="w-8 h-8 rounded-full object-cover mx-auto" />
          ...
          <svg v-if="isOpen" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 24 24"
        ...
        <div v-if="showMenu"
          class="absolute right-0 bottom-14 bg-white shadow-lg rounded-xl border border-gray-200 w-36 py-2 z-50 animate-fade">
          <button @click="logout"
            class="w-full text-left px-4 py-2 text-red-600 cursor-pointer flex items-center gap-2">
