<template>
  ...
  >
    <div class="flex flex-col mt-6 px-2 gap-1">
      <button
        @click="isOpen = !isOpen"
        class="p-2 mb-4 flex transition-all"
        :class="isOpen ? 'justify-between' : 'justify-center'"
      >
        <span v-if="isOpen" class="text-primary text-xl font-bold">
          ENTERRA AI
        </span>
    ...
    <div class="p-4 flex items-center gap-3">
      <img
        ...
        :class="isOpen ? 'w-10 h-10' : 'w-8 h-8 mx-auto'"
      />

      <div v-if="isOpen" class="flex-1">
        <p class="font-semibold text-gray-800">Admin User</p>
        <p class="text-xs text-gray-500">admin@mail.com</p>
      </div>
    </div>
  </div>
</template>
